...
{
    if(!Aies_bdd.isOpen()) {
qDebug() << "[CBdd::getActiveFlash]: BDD non ouverte";
        exit(1);
    } // if
...
qDebug("[CBdd::getPresCapteurs]: BDD non ouverte"); //verification d'accès à la bdd
        ...
        QSqlQuery Aies_query(req); // envoie de la requête à la BDD
        Aies_query.next();
        cs.append(Aies_query.value(0).toBool());
        cs.append(Aies_query.value(1).toBool());
        cs.append(Aies_query.value(2).toBool());
        cs.append(Aies_query.value(3).toBool());
    } // else
    return cs;
} // creationCacheVideo
